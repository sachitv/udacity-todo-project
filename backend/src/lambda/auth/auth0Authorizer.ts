...
const jwksUrl = 'https://sachit-udacity.us.auth0.com/.well-known/jwks.json'
...
async function getSigningKey(kid: string) {
  ...
  if (!signingKey) {
    throw new Error(`Could not find ${kid} in ${signingKeys}`)
  }

  logger.info('Got a valid signing key ', signingKey)
  return signingKey
}
